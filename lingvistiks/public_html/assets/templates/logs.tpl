...
<div class="menubar">
    ...
    <div class="page-title">
        <h1>{{lang.log_title}} <sup>{{ num|default('0') }}</sup></h1>
    </div>
</div>
...
<div class="datatables">
    <div class="content-wrapper">
        <div class="filters">
            <ul class="list-inline links">
                ...
                <li {% if not REQUEST.log_type %}class="active"{% endif %}>
                    <a href="{{ ABS_PATH_ADMIN_LINK }}?do=logs">
                        {{lang.log_all}}
                    </a>
                </li>
                {% for key, type in log_type %}
                    <li {% if REQUEST.log_type == key %}class="active"{% endif %}>
                        <a href="{{ ABS_PATH_ADMIN_LINK }}?do=logs&log_type={{ key }}">
                            {{ type }}
                        </a>
                    </li>
                {% endfor %}
	...
	<div class="content-wrapper search-section">
	    <form method="post" action="{{ ABS_PATH_ADMIN_LINK }}?do=logs" class="row">
			...
			<div class="col-lg-4 col-md-6 col-sm-6">
				...
				<div class="form-group">
					<label for="">{{ lang.search_group_log }}</label>
					<div class="clearfix"></div>
					<select class="selectpicker" name="log_type">
						<option value=""></option>
						{% for key, type in log_type %}
							<option value="{{ key }}" {% if REQUEST.log_type == key %}selected{% endif %} >{{ type }}</option>
		                {% endfor %}
					</select>
				</div>
			</div>
	        ...
	        <div class="actions">
				...
				{% if REQUEST.filter %}
		            <div class="filter_clear">
		                &nbsp;<a href="{{ ABS_PATH_ADMIN_LINK }}?do=logs"><i class="fa fa-times"></i> {{ lang.search_clear_btn }}</a>
		            </div>
		        {% endif %}
	        </div>
	    </form>
